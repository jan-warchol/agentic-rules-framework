...
class TestDeniedPaths:
    ...
    def test_denied_path_basic(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            decision, reason, _ = check_path(str(test_file), deny_list, base_dir)
    ...
    def test_denied_in_subdir(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            decision, _, _ = check_path(str(test_file), deny_list, base_dir)
            assert decision == "deny"
    ...
    def test_deeply_nested_paths(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            decision, _, _ = check_path(str(deep_file), deny_list, base_dir)
            assert decision == "deny"
    ...
    def test_denied_absolute_path(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            decision, _, _ = check_path(absolute_path, deny_list, some_other_dir)
            assert decision == "deny"
    ...
    def test_different_files_with_same_name(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            decision, _, _ = check_path(str(first_file), deny_list, base_dir)
            assert decision == "deny"

            # other/config.yaml should be allowed
            decision, _, _ = check_path(str(second_file), deny_list, base_dir)
            assert decision is None

            # config.yaml should be allowed
            decision, _, _ = check_path(str(third_file), deny_list, base_dir)
            assert decision is None
    ...
    def test_allowed_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            decision, _, _ = check_path("readme.md", deny_list, base_dir)
            assert decision is None
    ...
    def test_empty_deny_list(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            base_dir = Path(tmpdir)
            decision, _, _ = check_path("any_file.txt", [], base_dir)
            assert decision is None
    ...
    def test_relative_path_resolution_from_base_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            decision, _, _ = check_path(str(config1), deny_list, base_dir)
            assert decision == "deny"

            # config.yaml in project2 should be allowed (different absolute path)
            decision, _, _ = check_path(str(config2), deny_list, base_dir)
            assert decision is None
...
class TestProcessEditingTool:
    ...
    def test_denied_path(self):
        """Test that editing a single denied file returns deny decision and reason."""
        with tempfile.TemporaryDirectory() as tmpdir:
            base_dir = Path(tmpdir)
            ...
            decision, reason, _ = check_paths(
                [str(test_file)], {"deny_edits": deny_list}, base_dir
            )
    ...
    def test_multiple_paths_all_allowed(self):
        """Test that editing multiple allowed files returns None decision."""
        with tempfile.TemporaryDirectory() as tmpdir:
            base_dir = Path(tmpdir)
            rules_file = base_dir / "agent-rules.yaml"
            rules_file.touch()
    ...
    def test_multiple_paths_one_denied(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            denied_edits = [{"path": "protected.py", "reason": "Protected file"}]

            decision, reason, _ = check_paths(
                ["file1.py", str(protected_file), "file3.py"],
                {"deny_edits": denied_edits},
                base_dir,
            )
